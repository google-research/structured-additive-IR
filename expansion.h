...
class ExpansionPattern {
 public:
  ...
  virtual mlir::LogicalResult Match(const ComputeOpInstance &op) const = 0;
...
class TypedExpansionPattern : public ExpansionPattern {
 public:
  ...
  mlir::LogicalResult Match(const ComputeOpInstance &op) const final {
    if (op.is_copy()) return mlir::failure();
    auto cast_op = dyn_cast<OpTy>(op.GetDuplicatedOp());
